...
- **Hand-drawn SVG icon set** — 61 stroke glyphs on a 24×24 / 1.75px grid (`$lib/icons.ts` + `Icon.svelte`), rendered as `currentColor` SVGs throughout the nav, the board (toolbar, card chips, detail sheet, filters), and the accounting module. Comment-reaction emoji and email-synced description content are intentionally kept.
